...
export default function SurveyPanel({
  ...
  const canStart = !open && name.length > 0 && !stale;
  const submit = (e) => {
    e.preventDefault();
  ...
  return (
    <form className="survey-box" onSubmit={submit}>
      <div className="survey-head">
        <span className="survey-title">조사 차수</span>
      ...
      <div className="survey-info num">
        {open && current
          ...
          : `${today} · ${name ? `${name}에서 오늘 ${sameDay.length}차까지 진행` : '조사지를 먼저 적으세요'}`}
      </div>
      ...
      <p className="note">
        {open
          ? '차수를 종료해야 다음 차수를 시작할 수 있습니다. 종료하면 진행 중이던 측정은 기록되지 않습니다.'
          : '저수지를 바꿔 적으면 그곳의 1차부터 다시 시작합니다.'}
      </p>
    </form>
  );
}
