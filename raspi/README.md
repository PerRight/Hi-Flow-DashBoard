...
| `gps.py` | **부표 GPS 리더.** NMEA GGA/RMC 파싱, 체크섬 검증, 마지막 FIX 유지. 단독 실행하면 GPS 만 점검 |
| `feed.py` | CLI. 1 Hz 폴링 + GPS 스냅샷 + WebSocket 송신 + 현장 디버그 모드(`--probe` / `--once`) |
| `dec890.py` | **레거시/참고용.** 구형 DEC890(FC04·float) 리더. `--driver dec890` 으로만 쓰인다 |
| `requirements.txt` | `minimalmodbus`, `websockets`, `pyserial` |

> `hi_flow_collector.py`(GPS+센서 통합 시험 스크립트)의 GPS 부분이 `gps.py` 로 들어왔다.
> 센서 읽기는 검증된 `ects.py` 개별 읽기를 그대로 쓴다 (CLAUDE.md 4절, 2026-09-13).
> 그 스크립트는 송신 코드가 없어 단독으로는 대시보드에 아무것도 보내지 못한다 — 참고용으로만 둔다.
...
- GPS 는 **센서 틱을 막지 않는다.** 별도 스레드가 NMEA 를 계속 읽고, 송신 루프는
  그 시점의 최신 FIX 만 집어 간다. GPS 가 죽어도 수질값 송신은 계속된다.
- **FIX 가 끊겨도 마지막 좌표를 지우지 않고** `gps_fix:false` 로 보낸다(사용자 확정 2026-09-13).
  좌표를 버리면 HOLD 30초 중 순간적인 끊김만으로 그 지점이 히트맵에서 통째로 사라진다.
  대신 대시보드가 주의색 배너로 "현재 위치가 아님"을 표시한다.
- FIX 를 한 번도 못 잡았으면 `lat`/`lon` 은 `null` 이고, 서버는 **모의 좌표로 돌아가지 않는다.**
  가짜 보트가 저수지를 돌아다니는 것이 좌표 없는 것보다 나쁘다(6절).
- 첫 FIX 까지 **수십 초~수 분** 걸린다(콜드 스타트). 실외에서 하늘이 보이는 곳에 두고 기다린다.
  `python3 gps.py --port /dev/ttyAMA0` 로 서버·센서 없이 GPS 만 먼저 확인할 수 있다.
- 좌표를 코드에서 평활화·보정하지 않는다(4절). `round(…, 7)` 은 표현 자리수일 뿐이다.
